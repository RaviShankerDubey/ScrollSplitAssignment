package com.assignment.scrollsplitassignment.view.shimmerlayout;
...
public class ShimmerLayout extends FrameLayout {
    ...
    public void setMaskWidth(float maskWidth) {
        if (maskWidth <= MIN_MASK_WIDTH_VALUE || MAX_MASK_WIDTH_VALUE < maskWidth) {
            throw new IllegalArgumentException(String.format("maskWidth value must be higher than %d and less or equal to %d", MIN_MASK_WIDTH_VALUE, MAX_MASK_WIDTH_VALUE));
        }
    ...
    public void setGradientCenterColorWidth(float gradientCenterColorWidth) {
        if (gradientCenterColorWidth <= MIN_GRADIENT_CENTER_COLOR_WIDTH_VALUE || MAX_GRADIENT_CENTER_COLOR_WIDTH_VALUE <= gradientCenterColorWidth) {
            throw new IllegalArgumentException(String.format("gradientCenterColorWidth value must be higher than %d and less than %d", MIN_GRADIENT_CENTER_COLOR_WIDTH_VALUE, MAX_GRADIENT_CENTER_COLOR_WIDTH_VALUE));
        }
    ...
    private void createShimmerPaint() {
        ...
        LinearGradient gradient = new LinearGradient(0, yPosition,
                ...
                getGradientColorDistribution(), Shader.TileMode.CLAMP);
